...
function test() {
    ...
    //queryHandler("wrong","stuff",global_vars.currentServer,"users","","login","","POST", "");
    //queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"users","","login","","POST");

    //queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"parts","1","addStock","quantity=5","PUT")
    //queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"parts","2","","","PUT");
...
function login(username, password) {
    ...
    queryHandler(username, password, global_vars.currentServer, "users", "", "login", "", "POST", "", "");
}

function logout(username, password) {
    //console.log(username);
    //console.log(password);
    queryHandler(username, password, global_vars.currentServer, "users", "", "logout", "", "GET", "", "");
}

function logoutQuit(username, password) {
    //console.log(username);
    //console.log(password);
    queryHandler(username, password, global_vars.currentServer, "users", "", "logout", "", "GET", "logoutQuit", "");
}
...
function checkIn(itemListIn) {
    ...
    while (i < itemListIn.length) {
        ...
        queryHandler(global_vars.username, global_vars.realpass, global_vars.currentServer, "parts", itemListIn[i], "addStock", "quantity=1", "PUT", "", itemListIn[i]);
        i += 1
    }
}

function checkInLookup(item) {
    global_vars.checkInCheck = 0;
    queryHandler(global_vars.username, global_vars.realpass, global_vars.currentServer, "parts", item, "", "", "GET", "checkInLookup", "");
    item = item.toString()
    item = item.slice(0,-1)
    ...
    queryHandler(global_vars.username, global_vars.realpass, global_vars.currentServer, "stock_entries", "", "", stockstring, "GET", 'checkInHistory', item);
}
...
function checkOut(itemList) {
    ...
    while (i < itemList.length) {
        ...
        queryHandler(global_vars.username, global_vars.realpass, global_vars.currentServer, "parts", itemList[i], "removeStock", "quantity=1", "PUT", "", itemList[i]);
        i += 1
    }
}
...
function checkOutLookup(item) {
    ...
    queryHandler(global_vars.username, global_vars.realpass, global_vars.currentServer, "parts", item, "", "", "GET", "checkOutLookup", "");
    item = item.toString()
    item = item.slice(0,-1)
    var stockstring = "start=0&order=%5B%7B%22property%22%3A%22dateTime%22%2C%22direction%22%3A%22DESC%22%7D%5D&filter=%5B%7B%22subfilters%22%3A%5B%5D%2C%22property%22%3A%22part%22%2C%22operator%22%3A%22%3D%22%2C%22value%22%3A%22%2Fapi%2Fparts%2F" + item + "%22%7D%5D"
    queryHandler(global_vars.username, global_vars.realpass, global_vars.currentServer, "stock_entries", "", "", stockstring, "GET", 'checkOutHistory', item);
}

function lookUp(item) {
    queryHandler(global_vars.username, global_vars.realpass, global_vars.currentServer, "parts", item, "", "", "GET", "lookUp", "");
}
...
//    console.log('userstring: ', userstring);
    queryHandler("admin", "abc123pass", global_vars.currentServer,"users","","",userstring,"POST", "addUser", "");
}
...
function addPart(newItem, newLocation) {
    ...
    queryHandler("admin", "abc123pass", global_vars.currentServer, "parts", "", "", itemstring, "POST", "addPart", newItem);
    //queryHandler("admin", "abc123pass", global_vars.currentServer, "parts", "", "", '{"name":"Insertinoz","description":"","comment":"","minStockLevel":0,"status":"","needsReview":false,"partCondition":"","productionRemarks":"","internalPartNumber":"","metaPart":false,"category":{"@context":"/api/contexts/PartCategory","@id":"/api/part_categories/1","@type":"PartCategory","categoryPath":"Root Category","expanded":true,"name":"Root Category","description":null,"parentId":"@local-tree-root","index":0},"partUnit":{"@id":"/api/part_measurement_units/1","@type":"PartMeasurementUnit","name":"Pieces","shortName":"pcs","default":true},"footprint":null,"storageLocation":{"@id":"/api/storage_locations/5","@type":"StorageLocation","name":"tool_room_wallA","image":null,"categoryPath":"Root Category"},"stockLevels":[{"stockLevel":288,"price":0,"dateTime":null,"correction":false,"comment":null,"user":null}]}', "POST", "addPart", newItem);
}
...
function setStock(item, stocknumber) {
    ...
    //queryHandler("partMaster", "warehouseMaster",global_vars.currentServer,"parts","27","setStock","quantity=156","PUT", "", item);
    queryHandler("admin", "abc123pass", global_vars.currentServer, "parts", item, "setStock", stockstring, "PUT", "", item);
}

function getStockHistory(item) {

    //queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"stock_entries","","","start=0&order=%5B%7B%22property%22%3A%22dateTime%22%2C%22direction%22%3A%22DESC%22%7D%5D&filter=%5B%7B%22subfilters%22%3A%5B%5D%2C%22property%22%3A%22part%22%2C%22operator%22%3A%22%3D%22%2C%22value%22%3A%22%2Fapi%2Fparts%2F0005%22%7D%5D","GET", "lookupHistory", item);
    ...
    //console.log("stockstring: ", stockstring)
    queryHandler(global_vars.username, global_vars.realpass, global_vars.currentServer, "stock_entries", "", "", stockstring, "GET", 'lookupHistory', item);
...
function errorHandler(response, headers, command, command2, item) {
    ...
    switch(command) {
        ...
        case "addStock":
            ...
            global_vars.currentCheckInOut += 1;
            if (global_vars.currentCheckInOut == global_vars.totalCheckInOut) {
                end_page.checkInMsg();
            }
            end_page.addBadIn(item);
        ...
        case "removeStock":
            ...
            global_vars.currentCheckInOut += 1;
            if (global_vars.currentCheckInOut == global_vars.totalCheckInOut) {
                end_page.checkOutMsg();
            }
            end_page.addBadOut(item);
        ...
        default:
            switch(command2) {
                ...
                case "checkInLookup":
                    console.log("Couldn't lookup part for checkin");
...
//                    console.log("Low Stock: " + arr["lowStock"]);
                    ...
                    if (global_vars.lookupCheck == 0) {
                        global_vars.lookupCheck = 1;
                    }
                    else if (global_vars.lookupCheck == 1) {
                        lookup.lookupString();
                    }
...
//                    encode64(xhr.responseText);
                ...
                case "checkInLookup":
                    console.log("Checkin lookup Executed\n");
                    ...
                    if (global_vars.checkInCheck == 0) {
                        global_vars.checkInCheck = 1;
                    }
                ...
                case "checkInHistory":
                    console.log("Checkin history executed\n");
...
/* ******************************** CORRESPONDENCE TABLE && EXAMPLES ************************************
   ...
   MODIFY ITEM : CHANGE MODULE TO PARTS AND SPECIFY PART ID !
    ...
    queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"parts","1","addStock","quantity=666","PUT");


   // REMOVE STOCK

    queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"parts","1","removeStock","quantity=666","PUT");

   //SET STOCK

    queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"parts","1","setStock","quantity=114","PUT");
   ...
   // ADDING A USER
         ...
         queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"users","","",'{"newPassword":"pass","username":"user","admin":0}',"POST");

   // ADDING AN ITEM

        queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"parts","","",
   ...
   // MODIFYING AN ITEM
        ...
        queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"parts","1","",
    ...
    // LOGGING OUT
     ...
     queryHandler("partMaster","warehouseMaster",global_vars.currentServer,"users","","logout","","GET");
